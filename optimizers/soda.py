...
class SODA(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-3,
        ...
        defaults = dict(lr=lr, k=0, radius=radius, anchored=anchored,
    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if anchored and 'z_0' not in state.keys():
                    state['z_0'] = torch.clone(p.data)
                # Set states
                ...
                if anchored:
                    # x_{k+1} = 1/(k+2) x_0 + (k+1)/(k+2) x_k - lr * radius * lmo(m)
                    p.data.mul_(1-1/(k+2)).add_(state['z_0'], alpha=1/(k+2)).add_(update, alpha=-lr)
                else:
