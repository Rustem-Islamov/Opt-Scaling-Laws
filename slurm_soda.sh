#!/bin/bash
#SBATCH -J soda-base-model-tuning
...
#SBATCH --mem=160G
#SBATCH -o soda_train_logs/%x_%A.out
#SBATCH -e soda_train_logs/%x_%A.err
#SBATCH --exclude=gpu-08
...
# --- Logging setup ---
OUTDIR="soda_train_logs"
mkdir -p "$OUTDIR"

stamp="$(date +'%Y%m%d-%H%M%S')_${SLURM_JOB_ID}"
LOGFILE="${OUTDIR}/soda_${stamp}.log"
...
# --- Config manipulation ---
ORIG_CFG="./configs/soda.yaml"
NEW_CFG="./configs/temp/soda_${stamp}.yaml"
...
cfg["project"] = f"124M-CORRECT-SODA-budget{t_budget}"
cfg["warmdown_iters"] = int(0.28*cfg["num_iterations"])


run = cfg.get("run", "soda")
