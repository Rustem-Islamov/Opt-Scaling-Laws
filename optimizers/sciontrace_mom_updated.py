...
class ScionTrace(torch.optim.Optimizer):
    ...
    def report_stats(self, eps: float = 1e-12) -> Optional[Dict[str, float]]:
        ...
        delta1_star_local = 0.0
        delta1_fro_sq_local = 0.0
        
        delta2_star_local = 0.0
        delta2_fro_sq_local = 0.0
        
        mom_delta_star_local = 0.0
        mom_delta_fro_sq_local = 0.0
        ...
        for i in range(len(params)):
            ...
            if g1 is None:
                g1f = torch.zeros_like(mean_g)
            else:
                g1f = g1.to(dtype=torch.float32)
                
            if g2 is None:
                g2f = torch.zeros_like(mean_g)
            else:
                g2f = g2.to(dtype=torch.float32)
            ...
            delta1_fro_sq_local += float(torch.sum(delta1 * delta1).item())
            delta2_fro_sq_local += float(torch.sum(delta2 * delta2).item())
            ...
            delta_for_lmo = delta
            delta1_for_lmo = delta1
            delta2_for_lmo = delta2
            ...
            if device.type == "cuda" and delta_for_lmo.device.type != "cuda":
                ...
                delta1_for_lmo = delta1_for_lmo.to(device=device)
                delta2_for_lmo = delta2_for_lmo.to(device=device)

            lmo_delta = norm_backends[i].lmo(delta_for_lmo)
            lmo_delta1 = norm_backends[i].lmo(delta1_for_lmo)
            lmo_delta2 = norm_backends[i].lmo(delta2_for_lmo)
            
            # Ensure float32 for stable dot product
            lmo_delta = lmo_delta.to(dtype=torch.float32)
            lmo_delta1 = lmo_delta1.to(dtype=torch.float32)
            lmo_delta2 = lmo_delta2.to(dtype=torch.float32)
            
            delta_star_local += float(torch.sum(delta_for_lmo.to(dtype=torch.float32) * lmo_delta).item())
            delta1_star_local += float(torch.sum(delta1_for_lmo.to(dtype=torch.float32) * lmo_delta1).item())
            delta2_star_local += float(torch.sum(delta2_for_lmo.to(dtype=torch.float32) * lmo_delta2).item())

            
            # --- NEW: delta vs momentum buffer ---
            p = params[i]
            st = self.state.get(p, {}) if hasattr(self, "state") else {}
            buf = st.get("momentum_buffer", None)
    
            # If no momentum buffer yet (e.g., first step), treat it as 0.
            if buf is None:
                buf_f = torch.zeros_like(mean_g)
            else:
                buf_f = buf.detach().to(dtype=torch.float32)
                if buf_f.device != mean_g.device:
                    buf_f = buf_f.to(device=mean_g.device)
    
            mom_delta = mean_g - buf_f
    
            # Frobenius part
            mom_delta_fro_sq_local += float(torch.sum(mom_delta * mom_delta).item())
    
            # Star part via LMO (possibly move to CUDA for speed)
            mom_delta_for_lmo = mom_delta
            if device.type == "cuda" and mom_delta_for_lmo.device.type != "cuda":
                mom_delta_for_lmo = mom_delta_for_lmo.to(device=device)
    
            lmo_mom_delta = norm_backends[i].lmo(mom_delta_for_lmo).to(dtype=torch.float32)
            mom_delta_star_local += float(torch.sum(mom_delta_for_lmo.to(dtype=torch.float32) * lmo_mom_delta).item())
        ...
        if dist.is_initialized():
            ...
            delta1_star_tensor = torch.tensor(delta1_star_local, device=device)
            delta1_fro_sq_tensor = torch.tensor(delta1_fro_sq_local, device=device)
            
            delta2_star_tensor = torch.tensor(delta2_star_local, device=device)
            delta2_fro_sq_tensor = torch.tensor(delta2_fro_sq_local, device=device)
            
            mom_delta_star_tensor = torch.tensor(mom_delta_star_local, device=device)
            mom_delta_fro_sq_tensor = torch.tensor(mom_delta_fro_sq_local, device=device)
            ...
            dist.all_reduce(delta1_star_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(delta1_fro_sq_tensor, op=dist.ReduceOp.SUM)
            
            dist.all_reduce(delta2_star_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(delta2_fro_sq_tensor, op=dist.ReduceOp.SUM)
            
            dist.all_reduce(mom_delta_star_tensor, op=dist.ReduceOp.SUM)
            dist.all_reduce(mom_delta_fro_sq_tensor, op=dist.ReduceOp.SUM)
            ...
            delta1_star = delta1_star_tensor.item() / world_size
            delta1_fro_sq = delta1_fro_sq_tensor.item() / world_size
            
            delta2_star = delta2_star_tensor.item() / world_size
            delta2_fro_sq = delta2_fro_sq_tensor.item() / world_size
            
            mom_delta_star = mom_delta_star_tensor.item() / world_size
            mom_delta_fro_sq = mom_delta_fro_sq_tensor.item() / world_size
        else:
            Eg2 = Eg2_local
            ...
            delta1_star = delta1_star_local
            delta1_fro_sq = delta1_fro_sq_local
            
            delta2_star = delta2_star_local
            delta2_fro_sq = delta2_fro_sq_local
            
            mom_delta_star = mom_delta_star_local
            mom_delta_fro_sq = mom_delta_fro_sq_local
        ...
        delta1_fro = math.sqrt(max(delta1_fro_sq, 0.0))
        delta1_star_over_fro = float(delta1_star / (delta1_fro + eps))
        
        delta2_fro = math.sqrt(max(delta2_fro_sq, 0.0))
        delta2_star_over_fro = float(delta2_star / (delta2_fro + eps))
        
         # NEW: ||mean_g - momentum_buffer||_* / ||mean_g - momentum_buffer||_F
        mom_delta_fro = math.sqrt(max(mom_delta_fro_sq, 0.0))
        mom_delta_star_over_fro = float(mom_delta_star / (mom_delta_fro + eps))
        ...
        stats: Dict[str, float] = {
            ...
            "rho/delta_sample_0_fro": float(delta_fro),
            "rho/delta_sample_0_star": float(delta_star),
            "rho/noise_rho_sample_0": float(delta_star_over_fro),
            
            "rho/delta_sample_1_fro": float(delta1_fro),
            "rho/delta_sample_1_star": float(delta1_star),
            "rho/noise_rho_sample_1": float(delta1_star_over_fro),
            
            "rho/delta_sample_2_fro": float(delta2_fro),
            "rho/delta_sample_2_star": float(delta2_star),
            "rho/noise_rho_sample_2": float(delta2_star_over_fro),
            
            "rho/averaged_rho_over_samples": (float(delta2_star_over_fro) + float(delta1_star_over_fro) + float(delta_star_over_fro))/3,
            "rho/rho_over_averaged_norms": (float(delta_star) + float(delta1_star) + float(delta2_star)) / (float(delta_fro) + float(delta1_fro) + float(delta2_fro)),
            
            # NEW requested metric
            "mom/delta_mean_minus_buf_fro": float(mom_delta_fro),
            "mom/delta_mean_minus_buf_star": float(mom_delta_star),
            "mom/rho_mean_minus_buf": float(mom_delta_star_over_fro),
            
            "noise_step": self.n_steps
        }
    ...
    def track_rhos(self) -> Dict[str, float]:
        ...
        rhos["rho/rho_step"] = self.n_steps
