...
def make_optimal_wsd_batch_schedule(num_updates: int, warmup_updates: int,
                                    cooldown_updates: int, base_batch_size: int,
                                    batch_quantum: int) -> np.ndarray:
    """Paper Eq. (3.4), normalized to base_batch_size and rounded to batch_quantum.

    The LR warmup is included. The unrounded schedule has exactly the same total
    sample budget as a static base_batch_size run; rounding may change it slightly.
    """
    assert base_batch_size > 0 and batch_quantum > 0
    assert base_batch_size % batch_quantum == 0
    assert 0 <= warmup_updates < num_updates
    assert 0 < cooldown_updates < num_updates
    assert warmup_updates + cooldown_updates <= num_updates

    x = (np.arange(num_updates, dtype=np.float64) + 0.5) / num_updates
    w = warmup_updates / num_updates
    c = 1.0 - cooldown_updates / num_updates

    lr_shape = np.ones_like(x)
    if warmup_updates > 0:
        warm_mask = x < w
        lr_shape[warm_mask] = x[warm_mask] / w
    cooldown_mask = x > c
    lr_shape[cooldown_mask] = (1.0 - x[cooldown_mask]) / (1.0 - c)

    # Exact future LR area A(x) = integral_x^1 eta(s) ds for piecewise-linear WSD.
    future_area = np.empty_like(x)
    future_area[cooldown_mask] = (
        (1.0 - x[cooldown_mask]) ** 2 / (2.0 * (1.0 - c))
    )
    plateau_mask = ~(cooldown_mask | ((x < w) if warmup_updates > 0 else False))
    future_area[plateau_mask] = c - x[plateau_mask] + (1.0 - c) / 2.0
    if warmup_updates > 0:
        future_area[warm_mask] = (
            (w * w - x[warm_mask] ** 2) / (2.0 * w)
            + (c - w)
            + (1.0 - c) / 2.0
        )

    # Eq. (3.4): b(t) = eta(t) / sqrt(integral_t^T eta(s) ds).
    shape = lr_shape / np.sqrt(future_area)
    unrounded = base_batch_size * shape / shape.mean()
    rounded = np.floor(unrounded / batch_quantum + 0.5) * batch_quantum
    return np.maximum(rounded, batch_quantum).astype(np.int64)
...
try:
    ...
    # Compute the data-optimal batch schedule from Eq. (3.4) of Li, Bu & Xu.
    # On one GPU with device_batch_size=32, the global batch quantum is 32.
    batch_quantum = B * ddp_world_size
    configured_quantum = getattr(args, "batch_size_rounding", batch_quantum)
    assert configured_quantum == batch_quantum, (
        f"batch_size_rounding={configured_quantum} must equal device_batch_size * "
        f"world_size={batch_quantum} so every batch is realizable by accumulation"
    )
    if getattr(args, "dynamic_batch_size", True):
        global_batch_schedule = make_optimal_wsd_batch_schedule(
            args.num_iterations,
            args.warmup_iters,
            args.warmdown_iters,
            args.batch_size,
        ...
        global_batch_schedule = np.full(args.num_iterations, args.batch_size, dtype=np.int64)
    if master_process:
        print(
            f"global batch schedule: min={global_batch_schedule.min()}, "
            f"max={global_batch_schedule.max()}, mean={global_batch_schedule.mean():.3f}, "
            f"sum={global_batch_schedule.sum()}"
        )
    ...
    for step in tqdm(range(1, args.num_iterations + 1)):
        ...
        model.train()
        global_batch_size = int(global_batch_schedule[step - 1])
        train_accumulation_steps = global_batch_size // batch_quantum
