...
from optimizers.sciontrace import ScionTrace
...
class ExponentialStepSampler:
    def __init__(self, T=200, S_max=8, start_step_size=1, max_step_size=None):
        self.T = T
        self.S_max = S_max
        self.start_step_size = start_step_size
        self.max_step_size = max_step_size
        self.next_sample = 0  # next step index to sample

    def should_sample(self, n):
        if n >= self.next_sample:
            step_size = self.start_step_size * (self.S_max / self.start_step_size) ** (n / self.T)
            if self.max_step_size is not None:
                step_size = min(step_size, self.max_step_size)
            step_size = max(1, step_size)
            self.next_sample = n + int(step_size)
            return True
        return False
trace_step_sampler = ExponentialStepSampler(T=args.trace_step_T, S_max=args.trace_step_S, start_step_size=1, max_step_size=args.trace_step_max)
...
wandb.log({
            "run/start_val_loss": val_loss.item() if val_loss else None,
        })
...
for step in tqdm(range(args.num_iterations + 1)):
    ...
    for opt, sched in zip(optimizers, schedulers):
        opt_logs = opt.step(step=step, loss=avg_accumulated_loss, trace=True) # trace_step_sampler.should_sample(step)
        sched.step()
...
wandb.log({
            "run/final_val_loss": val_loss.item() if val_loss else None,
        })
