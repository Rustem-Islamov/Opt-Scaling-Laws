...
class ScionTrace(torch.optim.Optimizer):
    ...
    def report_stats(self, eps: float = 1e-12) -> Optional[Dict[str, float]]:
        """Compute mean/variance of stored stochastic gradients across all GPUs (DDP-safe)."""
        ...
        # Norm backends aligned with flattened parameter order (same order as track_stats()).
        # This lets us compute layer-wise LMO-based statistics in the same param order as stored samples.
        norm_backends = []
        for group in self.param_groups:
        ...
        # --- Local computation of E||g||^2 and ||E[g]||^2 ---
        sum_g2 = 0.0
        for s in samples:
            for g in s:
                if g is None:
        ...
        # --- New: ||G_t - g_t||_* / ||G_t - g_t||_F (G_t = mean grad, g_t = first sample) ---
        ...
        for i in range(len(params)):
            ...
            for s in samples:
                ...
                if g is None:
                    continue
                if acc is None:
                    acc = g.to(torch.float32).clone()
                else:
                    acc.add_(g.to(torch.float32))
            if acc is not None:
                mean_g2_local += float(torch.sum(acc * acc).item())

                # Compute delta = G_t - g_t for this parameter, where G_t is the averaged gradient
                # across the tracked window and g_t is the first sampled mini-batch gradient.
                mean_g = acc / m
                g0 = first_sample[i]
                if g0 is None:
                    g0f = torch.zeros_like(mean_g)
                else:
                    g0f = g0.to(torch.float32)
                delta = mean_g - g0f

                # Accumulate layer-wise star/"nuclear-like" inner product using the same LMO backend
                # as the optimizer for this parameter.
                delta_star_local += float(torch.sum(delta * norm_backends[i].lmo(delta)).item())
        ...
        # Ratio: ||G_t - g_t||_* / ||G_t - g_t||_F
        delta_fro = math.sqrt(max(delta_fro_sq, 0.0))
        delta_star_over_fro = delta_star / (delta_fro + eps)
        ...
        stats: Dict[str, float] = {
            ...
            "grad/G_t_g_t_star_over_G_t_g_t_fro":delta_star_over_fro,
            "grad/G_t_g_t_star":delta_star,
            "grad/G_t_g_t_fro":,delta_fro
        ...
        # store for logging on the next step(), then clear stored grads
    ...
    def track_rhos(self):
        ...
        for group_idx, group in enumerate(self.param_groups):
            ...
            for param_idx, p in enumerate(group['params']):
                ...
                # Apply momentum
                if momentum != 1:
                    total_mom_nuc += torch.sum( state['momentum_buffer']*norm_backend.lmo(state['momentum_buffer']) ).item()
                    total_mom_fro_sq += torch.sum(state['momentum_buffer']*state['momentum_buffer']).item()
                    
                #if p is None or p.ndim < 2:
                #    continue

                total_nuc += torch.sum(p * norm_backend.lmo(p)).item()
                total_fro_sq += torch.sum(p * p).item()
        ...
        if dist.is_initialized():
            device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
            total_nuc_tensor = torch.tensor([total_nuc], device=device)
            total_fro_sq_tensor = torch.tensor([total_fro_sq], device=device)
            total_mom_nuc_tensor = torch.tensor([total_mom_nuc], device=device)
            total_mom_fro_sq_tensor = torch.tensor([total_mom_fro_sq], device=device)
            #rho_max_tensor = torch.tensor([rho_max], device=device)
            ...
            #dist.all_reduce(rho_max_tensor, op=dist.ReduceOp.MAX)

            total_nuc = total_nuc_tensor.item()
            total_fro_sq = total_fro_sq_tensor.item()
            total_mom_nuc = total_mom_nuc_tensor.item()
            total_mom_fro_sq = total_mom_fro_sq_tensor.item()
            #rho_max = rho_max_tensor.item()

        rhos['rho/total'] = total_nuc / math.sqrt(total_fro_sq + 1e-12)
        rhos['rho/total_nuc'] = total_nuc
        rhos['rho/total_fro'] = math.sqrt(total_fro_sq)
        rhos['rho/total_mom_nuc'] = total_mom_nuc
        rhos['rho/total_mom_fro'] = math.sqrt(total_mom_fro_sq)
        #rhos['rho/max'] = rho_max
    ...
    def step(self, step, loss, trace=False):
        ...
        log_stats = {}
        if trace:
            if step > 1 and step % self.trace_stats_every == 0:
                log_stats = self.report_stats()
            else:
                self.track_stats(cur_loss=loss, store_on_cpu=True)
